...
import {
  type TryOnProvider,
  type TryOnProviderMetadata,
  type TryOnProviderStatusResult,
  type TryOnProviderSubmitInput,
  type TryOnProviderSubmitResult,
} from "./try-on-provider.js";
...
export class FashnTryOnProvider implements TryOnProvider {
  ...
  async submit(
    input: TryOnProviderSubmitInput,
  ): Promise<TryOnProviderSubmitResult> {
  ...
  async poll(providerPredictionId: string): Promise<TryOnProviderStatusResult> {
    const client = this.getClient();
