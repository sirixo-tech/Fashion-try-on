...
- Try Another Garment retains a compatible accepted model/person photo and its
  ...
  before generation with simple customer guidance.
- Category selector buttons use immediate selected state: selected is SelfX
  orange with white text and orange border; inactive is white/light with dark
  text and neutral border. Icon and label are centered and selected text must
  never briefly become invisible.
