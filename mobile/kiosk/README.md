...
customer-friendly garment picker/preview, local camera discovery, preview,
...
operator PIN access, advisory post-capture image quality checks, temporary
local garment images, async SelfX Try-On submission/polling and generated-result display.
...
- KIOSK-3A starts the customer Try-On flow with a temporary native garment
  image picker and preview. Normal customer UI must not show raw file paths,
  KIOSK milestone labels, garment type overrides or photo-style controls.
  Product Catalog browsing, captured garment input, Shopify/WooCommerce sources
  and remote asset selection remain future work.
...
2. Start Try-On, choose a garment image with the native picker and confirm the
   preview.
...
4. Choose a supported garment image and confirm its preview appears without a
   raw path field or technical garment controls.
5. Continue to the photo source screen.
